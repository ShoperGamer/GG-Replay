...
from inference.uvr.error_handling import ERROR_MAPPER, WINDOW_SIZE_ERROR
from inference.uvr.lib_v5 import spec_utils
from inference.uvr.lib_v5.vr_network import nets, nets_new
...
class SeparateAttributes:
    def __init__(
        ...
        if model_data.process_method == MDX_ARCH_TYPE:
            ...
            self.primary_model_name, self.primary_sources = self.model_basename, []
        ...
        if model_data.process_method == DEMUCS_ARCH_TYPE:
            self.demucs_stems = (
                model_data.demucs_stems if not main_process_method in [MDX_ARCH_TYPE, VR_ARCH_TYPE] else None
            )
    ...
    def run_mixer(self, mix, sources):
        try:
            ...
            logger.error("Mixer Failed: ", message)
            final_source = sources
...
class SeparateMDX(SeparateAttributes):
    ...
    def separate(self):
        if self.is_mdx_ckpt:
            ...
            self.model_run = separator.load_from_checkpoint(self.model_path, map_location=self.device).eval()
        else:
            print(f"Loading ONNX Model - device {ort.get_device()} with providers {config.ort_providers}")
            inference_session = ort.InferenceSession(self.model_path, providers=config.ort_providers)
            print(f"Using ORT device: {ort.get_device()}")

            def model_run(spek):
                start = timer()
                nonlocal inference_session
                try:
        ...
        if self.secondary_stem == VOCAL_STEM:
            secondary_stem_path, primary_stem_path = primary_stem_path, secondary_stem_path
    ...
    def initialize_mix(self, mix, is_ckpt=False):
        ...
        mix_waves = torch.tensor(mix_waves, dtype=torch.float32, device=self.device)
    ...
    def run_model(self, mix, is_ckpt=False, is_match_mix=False):
        ...
        if is_match_mix:
            ...
            spec_pred = (
                -self.model_run(-spek) * 0.5 + self.model_run(spek) * 0.5 if self.is_denoise else self.model_run(spek)
            )

        if is_ckpt:
            pred__cpu: torch.Tensor = self.istft(spec_pred).cpu()
            return pred__cpu.detach().numpy()
        else:
            x = torch.tensor(spec_pred, device=self.device)
            return self.istft(x)[:, :, self.trim : -self.trim].transpose(0, 1).reshape(2, -1).cpu().numpy()
    ...
    def istft(self, x, freq_pad=None) -> torch.Tensor:
        freq_pad = self.freq_pad.repeat([x.shape[0], 1, 1, 1]) if freq_pad is None else freq_pad
        if config.device == "mps":
            cpu = torch.device("cpu")
            x = x.to(cpu)
            freq_pad = freq_pad.to(cpu)
            self.window = self.window.to(cpu)
...
class SeparateDemucs(SeparateAttributes):
    ...
    def separate(self):
        ...
        if self.demucs_stems == ALL_STEMS:
            ...
            if not isinstance(self.primary_source, np.ndarray):
                self.primary_source = spec_utils.normalize(
                    source[self.demucs_source_map[self.primary_stem]], self.is_normalization
                ).T
            self.primary_source_map = {self.primary_stem: self.primary_source}
            self.write_audio(primary_stem_path, self.primary_source, samplerate)
...
class SeparateVR(SeparateAttributes):
    ...
    def separate(self):
        ...
        primary_stem_path = os.path.join(self.export_path, f"vocals.wav")
        secondary_stem_path = os.path.join(self.export_path, f"no_vocals.wav")
        logger.info(f"{SAVING_STEM[0]}{self.primary_stem}{SAVING_STEM[1]} to {primary_stem_path}")
        if not isinstance(self.primary_source, np.ndarray):
            wav = self.spec_to_wav(y_spec)
            self.primary_source = spec_utils.normalize(wav, self.is_normalization).T
        ...
        if self.secondary_stem != NO_OTHER_STEM and self.primary_stem != NO_OTHER_STEM:
            logger.info(f"{SAVING_STEM[0]}{self.secondary_stem}{SAVING_STEM[1]} to {secondary_stem_path}")
            if not isinstance(self.secondary_source, np.ndarray):
                ...
                if not self.model_samplerate == 44100:
                    input_tensor = self.secondary_source.T
