...
def normalize(wave, is_normalize=False):
    ...
    if maxv > 1.0:
        ...
        if is_normalize:
            logger.info(f"The result was normalized.")
            wave /= maxv
        else:
            logger.info(f"The result was not normalized.")
    else:
...
def merge_artifacts(y_mask, thres=0.01, min_range=64, fade_size=32):
    ...
    try:
        ...
        logger.info("Post Process Failed: ", message)
...
def spectrogram_to_wave(spec, hop_length, mid_side, mid_side_b2, reverse, clamp=False):
    ...
    if reverse:
        ...
        return np.asfortranarray(
            [np.add(wave_right / 1.25, 0.4 * wave_left), np.subtract(wave_left / 1.25, 0.4 * wave_right)]
        )
    else:
...
def cmb_spectrogram_to_wave(spec_m, mp, extra_bins_h=None, extra_bins=None):
    ...
    for d in range(1, bands_n + 1):
        ...
        offset += h
        hl = bp["hl"]
        mid_side = mp.param["mid_side"]
        mid_side_b2 = mp.param["mid_side_b2"]
        reverse = mp.param["reverse"]
        if d == bands_n:  # higher
            ...
            if bands_n == 1:
                wave = spectrogram_to_wave(spec_s, hl, mid_side, mid_side_b2, reverse)
            else:
                wave = np.add(
                    wave,
                    spectrogram_to_wave(spec_s, hl, mid_side, mid_side_b2, reverse),
                )
        else:
            ...
            if d == 1:  # lower
                ...
                to_wave = spectrogram_to_wave(spec_s, hl, mid_side, mid_side_b2, reverse)
                # Iterate over the sub-arrays
                for i in range(to_wave.shape[0]):
                    # For each sub-array:
                    x = np.arange(to_wave.shape[1])

                    # Mask for valid (not NaN and not infinite) entries
                    valid_mask = ~np.isnan(to_wave[i]) & ~np.isinf(to_wave[i])

                    # Interpolate using the valid entries
                    to_wave[i] = np.interp(x, x[valid_mask], to_wave[i][valid_mask])
                ...
                # filter nans from wave2
                # Iterate over the sub-arrays
                for i in range(wave2.shape[0]):
                    # For each sub-array:
                    x = np.arange(wave2.shape[1])

                    # Mask for valid (not NaN and not infinite) entries
                    valid_mask = ~np.isnan(wave2[i]) & ~np.isinf(wave2[i])

                    # Interpolate using the valid entries
                    wave2[i] = np.interp(x, x[valid_mask], wave2[i][valid_mask])

                wave = librosa.resample(wave2, sample_rate, sr)
